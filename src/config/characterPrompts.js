...
// 角色回复的prompt生成函数
export function generateCharacterResponsePrompt(character, userMessage) {
    // 使用配置中的基础模板，替换占位符
    ...
    // 添加角色特定指令
    const characterSpecificInstruction = characterPromptConfig.characterSpecificInstructions[character.id];
...
// 玩家自动回复的prompt生成函数
export function generatePlayerAutoResponsePrompt(character, characterMessage) {
    // 基础prompt模板
...
// 获取通用的角色回复降级方案
export function getFallbackRepliesForCharacter(character) {
