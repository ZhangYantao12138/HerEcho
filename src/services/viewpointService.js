...
import { VIEWPOINT_MAPPING, VIEWPOINT_DESCRIPTIONS, getViewpointDescriptionByKey } from '../config/viewpointConfig';
// 导出视角映射以便其他模块使用
 ...
 */
export function checkViewpointRelation(character) {
 ...
 */
export function getViewpointPrompt(character, message) {
    // 检查是否有特定的视角关系
    ...
    // 获取视角提示
    const viewpointGuidance = playerPromptConfig.viewpointGuidance[viewpointRelation.promptKey];
    // 使用详细生成方式，但添加视角特定指导
 ...
 * @returns 视角信息描述
 */
export function getViewpointDescription(characterId) {
 ...
 * @returns 可用的视角关系列表
 */
export function getAvailableViewpoints(characterId) {
    // 返回所有可用的视角关系
 ...
 * @returns 当前视角关系
 */
export function getCurrentViewpoint(characterId) {
