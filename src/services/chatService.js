 ...
 * @returns API响应
 */
async function callDeepSeekAPI(messages, temperature = systemPromptConfig.globalAISettings.defaultTemp) {
 ...
 * @returns 角色回复
 */
export async function generateCharacterReply(characterId, message, retryCount = 0) {
    try {
 ...
 * @returns 玩家视角的回复
 */
export async function generatePlayerReply(characterId, message) {
    const character = getCharacterById(characterId);
    if (!character) {
        throw new Error(`Character not found: ${characterId}`);
    }
 ...
 * @returns 自动回复选项列表
 */
export async function generateAutoReplies(characterId, message) {
    const character = getCharacterById(characterId);
    if (!character) {
        throw new Error(`Character not found: ${characterId}`);
    }
 ...
 * @returns 对话历史
 */
export async function getCharacterHistory(characterId) {
    const character = getCharacterById(characterId);
    if (!character) {
        throw new Error(`Character not found: ${characterId}`);
    }
 ...
 * @returns 返回新的AI回复
 */
export async function refreshAIResponse(characterId, lastUserMessage) {
    // 检查是否有用户消息可以刷新
